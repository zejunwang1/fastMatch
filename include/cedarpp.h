...
namespace cedar {
  ...
  public:
    ...
    template <typename T>
    size_t commonPrefixSearch(const char* key, size_t len, T* result, size_t result_len, size_t from = 0) const {
      size_t num = 0;
      for (size_t pos = 0; pos < len; ) {
        if (num == result_len)  return num;
        union { int i; value_type x; } b;
        ...
        if (b.i == CEDAR_NO_VALUE) continue;
        if (b.i == CEDAR_NO_PATH)  return num;
        _set_result (result, b.x, pos, from); 
        ++num;
      }
      return num;
    }
    ...
    int build(size_t num, const std::vector<std::string>& key, const value_type* val = 0) {
      for (size_t i = 0; i < num; ++i)
        update (key[i].c_str(), key[i].size(), val ? val[i] : value_type (i));
      return 0;
    }
    template <typename T>
